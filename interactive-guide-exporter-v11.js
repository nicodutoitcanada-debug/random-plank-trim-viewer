  ...
  try {
    ...
    function addStyles() {
      ...
      style.textContent = `
        ...
        #${BUTTON_ID} img{width:31px;height:31px;flex:0 0 31px;display:block;object-fit:contain;pointer-events:none;user-select:none}
    ...
    function createButton() {
      ...
      const actions = (pdfButton && pdfButton.parentElement) || document.querySelector('.generatedPlanActions');
      if (!actions) return null;
      const button = document.createElement('button');
      button.id = BUTTON_ID;
      button.type = 'button';
      button.setAttribute('data-show-phase', 'generated');
      button.setAttribute('aria-label', 'Generate custom installation guide');
      button.title = 'Generate custom installation guide';
      button.innerHTML = `
        <img src="download pdf.png" alt="" aria-hidden="true">
        <span class="generatedPlanDownloadLabel">GENERATE CUSTOM INSTALLATION GUIDE</span>`;
      button.className = 'generatedPlanActionButton generatedPlanDownloadIconButton isAvailable';
    ...
    function requestProjectName() {
      return new Promise((resolve) => {
        ...
        overlay.innerHTML = `<div class="igNameCard" role="dialog" aria-modal="true" aria-label="Project name"><div class="igNameTitle">Custom Installation Guide</div><div class="igNameBody">Enter the project name for this Random Plank installation guide.</div><input class="igNameInput" autocomplete="off" placeholder="Project name"><div class="igNameError">Please enter a project name.</div><div class="igNameActions"><button class="igNameBtn cancel" type="button">CANCEL</button><button class="igNameBtn primary generate" type="button">GENERATE GUIDE</button></div></div>`;
    ...
    function getPlanSnapshot() {
      try {
        return window.LuxGeneratedPlanBridge && typeof window.LuxGeneratedPlanBridge.getSnapshot === 'function'
          ? window.LuxGeneratedPlanBridge.getSnapshot()
          : null;
      } catch (_) {
        return null;
      }
    ...
    function formatRunSequence(run) {
      const labels = Array.isArray(run?.sequenceLabels) ? run.sequenceLabels : [];
      if (!labels.length) return 'Follow the red installation direction shown in this segment.';
      const compressed = [];
      ...
      return compressed.map((item) => item.count > 1 ? `${item.count}× ${item.label}` : item.label).join(' → ');
    }
    ...
    function capturePlanSvg() {
      ...
      if (cloneViewport) {
        ...
        cloneViewport.classList.remove('hasSegmentFocus');
      }
      clone.querySelectorAll('.isSegmentDimmed').forEach((n) => n.classList.remove('isSegmentDimmed'));
    ...
    async function optionalAssetDataUrl(file) {
      try { return await blobToDataUrl(await fetchBlob(file)); }
      catch (_) { return ''; }
    }
    ...
    async function buildInteractiveHtml(projectName) {
      ...
      const installRuns = snapshot && Array.isArray(snapshot.installRuns) ? snapshot.installRuns : [];
      const logoData = await blobToDataUrl(await fetchBlob(FILES.logo));
      const zoomOutIconData = await optionalAssetDataUrl('zoom out.png');
      const zoomInIconData = await optionalAssetDataUrl('zoom in.png');
      const sections = [
      ...
      const runButtons = installRuns.length > 1
        ? `<button class="segmentBtn isActive" type="button" data-segment="">ALL</button>${installRuns.map((run) => `<button class="segmentBtn" type="button" data-segment="${htmlEscape(run.id)}">${htmlEscape(run.label)}</button>`).join('')}`
        : '';
      const runFlows = installRuns.length
        ? installRuns.map((run) => `<div class="flowLine" data-flow-segment="${htmlEscape(run.id)}"><strong>${htmlEscape(run.label)}:</strong> ${htmlEscape(formatRunSequence(run))}</div>`).join('')
        : `<div class="flowLine"><strong>INSTALLATION FLOW:</strong> Follow the red installation direction shown in the plan.</div>`;
      const runJson = JSON.stringify(installRuns.map((run) => ({
        id: run.id,
        label: run.label,
        sequenceLabels: run.sequenceLabels || [],
        bounds: run.bounds || null,
        visualBounds: run.visualBounds || run.bounds || null
      }))).replace(/</g, '\\u003c');
...
:root{color-scheme:dark;--red:${RED};--bg:#0a0b0c}*{box-sizing:border-box}html{scroll-behavior:smooth;background:var(--bg)}body{margin:0;font-family:Arial,sans-serif;color:#fff;background:radial-gradient(circle at 50% -12%,#2b2d30 0,#131416 42%,#090a0b 100%)}.topBar{position:sticky;top:0;z-index:20;display:flex;justify-content:space-between;align-items:center;padding:12px 18px;background:rgba(8,9,10,.86);backdrop-filter:blur(15px);border-bottom:1px solid rgba(255,255,255,.1)}.topTitle{font-size:12px;font-weight:800}.actions{display:flex;gap:8px}.btn{border:1px solid rgba(255,255,255,.16);border-radius:999px;padding:10px 15px;background:rgba(255,255,255,.08);color:#fff;font-weight:800;cursor:pointer}.btn.primary{background:var(--red);border-color:transparent}.btn:disabled{opacity:.45}.cover{min-height:calc(100vh - 60px);padding:48px 34px 34px}.coverHeader{width:min(1160px,100%);margin:auto;display:flex;justify-content:space-between;align-items:flex-start;gap:28px}.project{margin:0;font-size:clamp(28px,4vw,52px);line-height:.95;font-weight:900;text-transform:uppercase}.subtitle{margin-top:7px;color:rgba(255,255,255,.62);font-size:18px;text-transform:uppercase}.logo{width:min(180px,24vw);max-height:76px;object-fit:contain}.planCard{position:relative;width:min(1180px,100%);height:min(68vh,760px);min-height:430px;margin:34px auto 0;padding:20px;border:1px solid rgba(255,255,255,.14);border-radius:26px;background:rgba(3,4,5,.70);box-shadow:0 28px 90px rgba(0,0,0,.34);overflow:hidden;touch-action:none}.planStage{position:absolute;inset:0;padding:20px;overflow:hidden}.interactivePlanSvg{width:100%;height:100%;display:block;user-select:none;touch-action:none;cursor:grab}.interactivePlanSvg.isPanning{cursor:grabbing}.interactivePlanSvg .generatedPlanContextWall{stroke:rgba(255,255,255,.13);stroke-width:6;stroke-linecap:round;fill:none;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanBaseWall{stroke:rgba(255,255,255,.33);stroke-width:5;fill:none;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanFlow{stroke:var(--red)!important;stroke-width:5;stroke-linecap:round;fill:none;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanFlowMotion{stroke:rgba(255,255,255,.88)!important;stroke-width:2;stroke-linecap:round;stroke-dasharray:7 18;fill:none;opacity:.9;animation:igFlowDash 1.05s linear infinite;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanArrowHead{fill:var(--red)!important;stroke:var(--red)!important;stroke-width:3.5;stroke-linejoin:round;stroke-linecap:round}@keyframes igFlowDash{to{stroke-dashoffset:-50}}.interactivePlanSvg .generatedPlanRpPlank.isGenerated{stroke:rgba(255,255,255,.17)!important;stroke-width:7;stroke-linecap:round;transition:stroke .24s ease,filter .24s ease;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanRpPlank.isGenerated.installLit{stroke:#fff!important;filter:drop-shadow(0 0 7px rgba(255,255,255,.5))}.interactivePlanSvg [data-install-run-id]{transition:opacity .25s ease,filter .25s ease}.interactivePlanSvg .segmentDimmed{opacity:.09!important;filter:grayscale(1)}.interactivePlanSvg .generatedPlanHotspot.segmentDimmed{pointer-events:none}.interactivePlanSvg .generatedPlanHotspot{cursor:pointer;transform-box:fill-box;transform-origin:center;outline:none!important;-webkit-tap-highlight-color:transparent}.interactivePlanSvg .generatedPlanHotspot:focus,.interactivePlanSvg .generatedPlanHotspot:focus-visible{outline:none!important}.interactivePlanSvg .generatedPlanHotspotCore{fill:#111;stroke:#fff;stroke-width:4;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanHotspotDot{fill:#fff}.interactivePlanSvg .generatedPlanHotspotConnector{stroke:rgba(255,255,255,.88);stroke-width:1.8;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanHotspotLabelBg{fill:rgba(0,0,0,.82);stroke:rgba(128,128,128,1);stroke-width:1;transition:.18s ease;vector-effect:non-scaling-stroke}.interactivePlanSvg .generatedPlanHotspotLabel{fill:#fff;font-size:12px;font-weight:800;text-anchor:middle}.interactivePlanSvg .generatedPlanHotspot:hover .generatedPlanHotspotCore{fill:var(--red);transform:scale(1.14);transform-box:fill-box;transform-origin:center}.interactivePlanSvg .generatedPlanHotspot:hover .generatedPlanHotspotLabelBg,.interactivePlanSvg .generatedPlanHotspot:focus .generatedPlanHotspotLabelBg,.interactivePlanSvg .generatedPlanHotspot:focus-visible .generatedPlanHotspotLabelBg{fill:rgba(213,43,55,.92)}.interactivePlanSvg .generatedPlanHotspot:focus .generatedPlanHotspotCore,.interactivePlanSvg .generatedPlanHotspot:focus-visible .generatedPlanHotspotCore{fill:var(--red)}.planSegments{position:absolute;left:18px;top:18px;z-index:4;display:flex;flex-wrap:wrap;gap:6px;max-width:calc(100% - 220px)}.segmentBtn{min-height:31px;border:1px solid rgba(255,255,255,.17);border-radius:999px;background:rgba(0,0,0,.58);color:rgba(255,255,255,.82);backdrop-filter:blur(9px);font:800 9px Arial,sans-serif;letter-spacing:.035em;padding:0 11px;cursor:pointer}.segmentBtn:hover{background:rgba(255,255,255,.13);color:#fff}.segmentBtn.isActive{background:rgba(213,43,55,.88);color:#fff;border-color:rgba(255,255,255,.24)}.planTools{position:absolute;right:18px;top:18px;z-index:4;display:flex;align-items:center;gap:3px}.planTool{width:34px;height:34px;padding:0;border:0;border-radius:0;background:transparent;color:#fff;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:opacity .18s ease,filter .18s ease,transform .18s ease}.planTool:hover{background:transparent;opacity:.78;filter:drop-shadow(0 0 6px rgba(255,255,255,.42));transform:translateY(-1px)}.planTool img{width:25px;height:25px;object-fit:contain;display:block;pointer-events:none;user-select:none}.planFitTool{min-height:32px;margin-left:7px;padding:0 12px;border:1px solid rgba(255,255,255,.17);border-radius:999px;background:rgba(0,0,0,.58);color:#fff;backdrop-filter:blur(9px);font:800 9px Arial,sans-serif;letter-spacing:.035em;cursor:pointer}.planFitTool:hover{background:rgba(255,255,255,.13)}.flowList{width:min(1020px,100%);margin:26px auto 0;display:grid;gap:7px;color:rgba(255,255,255,.78);font-size:14px;line-height:1.55}.flowLine strong{color:#fff}.flowLine.isHidden{display:none}.copy{width:min(1020px,100%);margin:14px auto 0;color:rgba(255,255,255,.78);font-size:14px;line-height:1.65}.callout{margin-top:15px;padding:14px 16px;border-left:3px solid var(--red);background:rgba(213,43,55,.08);border-radius:0 12px 12px 0;font-weight:700}.disclaimer{width:min(1120px,100%);margin:24px auto 0;color:rgba(255,255,255,.46);font-size:10px;line-height:1.55}.manualSection{width:min(1180px,calc(100% - 34px));margin:34px auto;padding:24px;border:1px solid rgba(255,255,255,.12);border-radius:24px;background:rgba(20,21,23,.95);box-shadow:0 20px 70px rgba(0,0,0,.28);scroll-margin-top:76px}.sectionHeading{display:flex;justify-content:space-between;align-items:flex-end;gap:18px;margin-bottom:18px}.sectionHeading span{color:var(--red);font-size:9px;font-weight:800;letter-spacing:.12em}.sectionHeading h2{margin:4px 0 0;font-size:24px}.sectionHeading a{color:rgba(255,255,255,.72);font-size:9px;font-weight:800;text-decoration:none}.pdfPages{display:flex;flex-direction:column;align-items:center;gap:18px}.loading{padding:56px 18px;color:rgba(255,255,255,.5);display:flex;align-items:center;justify-content:center;gap:12px}.loading:before{content:'';width:22px;height:22px;border:3px solid rgba(255,255,255,.14);border-top-color:var(--red);border-radius:50%;animation:igLoadSpin .8s linear infinite}@keyframes igLoadSpin{to{transform:rotate(360deg)}}.pdfPage{width:min(100%,1000px);background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 16px 42px rgba(0,0,0,.3)}.pdfPage canvas{display:block;width:100%;height:auto;border-radius:inherit}.fallback{padding:32px;text-align:center}.fallback a{display:inline-flex;margin-top:12px;padding:10px 16px;border-radius:999px;background:var(--red);color:#fff;text-decoration:none;font-size:10px;font-weight:800}@media(max-width:720px){.cover{padding:26px 14px}.planCard{padding:9px;min-height:380px}.planStage{padding:9px}.planSegments{left:10px;top:10px;max-width:calc(100% - 104px)}.planTools{right:10px;top:10px}.manualSection{padding:14px}.logo{width:128px}}@media print{html,body{background:#fff!important;color:#111!important}.topBar,.sectionHeading,.planSegments,.planTools{display:none!important}.cover{min-height:auto;background:#e9e9e9!important;color:#111!important;break-after:page}.subtitle,.copy,.flowList{color:#333!important}.disclaimer{color:#555!important}.planCard{height:auto;min-height:0;aspect-ratio:16/10;background:#141516!important;box-shadow:none}.planStage{position:absolute}.manualSection{width:100%;margin:0;padding:0;border:0;background:#fff!important;box-shadow:none}.pdfPages{gap:0}.pdfPage{width:100%;border-radius:0;box-shadow:none;break-after:page}}
</style></head><body><div class="topBar"><div class="topTitle">${htmlEscape(projectName)} · Interactive Installation Guide</div><div class="actions"><button class="btn" id="viewPlan">VIEW PLAN</button><button class="btn primary" id="printGuide" disabled>PREPARING GUIDE...</button></div></div><main><section class="cover" id="interactive-plan"><div class="coverHeader"><div><h1 class="project">${htmlEscape(projectName)}</h1><div class="subtitle">Interactive Installation Guide</div></div><img class="logo" src="${logoData}" alt="LUX"></div><div class="planCard"><div class="planStage">${planSvg}</div><div class="planSegments">${runButtons}</div><div class="planTools"><button class="planTool" id="planZoomOut" type="button" aria-label="Zoom out">${zoomOutIconData ? `<img src="${zoomOutIconData}" alt="">` : '−'}</button><button class="planTool" id="planZoomIn" type="button" aria-label="Zoom in">${zoomInIconData ? `<img src="${zoomInIconData}" alt="">` : '+'}</button><button class="planFitTool" id="planFit" type="button" aria-label="Fit plan">FIT PLAN</button></div></div><div class="flowList" id="flowList">${runFlows}</div><div class="copy"><div class="callout">Important: read the GENERAL section in full before beginning installation. Click any marked intersection in the plan to jump directly to the matching installation condition below.</div></div><div class="disclaimer"><strong>PLANNING / SITE VERIFICATION DISCLAIMER:</strong> ${htmlEscape(disclaimer)}</div></section>${sectionMarkup}</main><script id="runData" type="application/json">${runJson}</script><script>
(function(){const PDF_JS='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.min.js',PDF_WORKER='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174/pdf.worker.min.js',printBtn=document.getElementById('printGuide'),svg=document.querySelector('.interactivePlanSvg'),planStage=document.querySelector('.planStage'),planCard=document.querySelector('.planCard'),runData=JSON.parse(document.getElementById('runData').textContent||'[]');document.getElementById('viewPlan').onclick=()=>document.getElementById('interactive-plan').scrollIntoView({behavior:'smooth'});function b64(s){const raw=atob((s||'').replace(/\\s+/g,'')),a=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)a[i]=raw.charCodeAt(i)&255;return a}function load(){if(window.pdfjsLib)return Promise.resolve(window.pdfjsLib);return new Promise((res,rej)=>{const s=document.createElement('script');s.src=PDF_JS;s.onload=()=>window.pdfjsLib?res(window.pdfjsLib):rej(new Error('PDF renderer did not initialize.'));s.onerror=()=>rej(new Error('PDF renderer could not be loaded.'));document.head.appendChild(s)})}function fallback(sec,data,msg){const p=sec.querySelector('.pdfPages');p.innerHTML='';const d=document.createElement('div');d.className='fallback';d.textContent=msg||'This section could not be rendered.';try{const a=document.createElement('a');a.textContent='OPEN SECTION PDF';a.target='_blank';a.href=URL.createObjectURL(new Blob([b64(data)],{type:'application/pdf'}));d.appendChild(document.createElement('br'));d.appendChild(a)}catch(e){}p.appendChild(d)}async function render(lib,sec){const data=sec.querySelector('.pdfData').innerHTML.trim(),pages=sec.querySelector('.pdfPages');try{const pdf=await lib.getDocument({data:b64(data)}).promise;pages.innerHTML='';for(let n=1;n<=pdf.numPages;n++){const page=await pdf.getPage(n),raw=page.getViewport({scale:1}),css=Math.min(1000,Math.max(320,innerWidth-90)),os=Math.min(2,devicePixelRatio||1),vp=page.getViewport({scale:(css/raw.width)*os}),c=document.createElement('canvas');c.width=Math.ceil(vp.width);c.height=Math.ceil(vp.height);c.style.width=Math.round(vp.width/os)+'px';c.style.height=Math.round(vp.height/os)+'px';const w=document.createElement('div');w.className='pdfPage';w.appendChild(c);pages.appendChild(w);await page.render({canvasContext:c.getContext('2d',{alpha:false}),viewport:vp}).promise}}catch(e){fallback(sec,data,e.message)}}document.querySelectorAll('.interactivePlanSvg .generatedPlanHotspot').forEach(h=>{const go=()=>{const t=document.getElementById('guide-section-'+h.getAttribute('data-hotspot-key'));if(t)t.scrollIntoView({behavior:'smooth',block:'start'})};h.onclick=go;h.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go()}}});const tiles=[...document.querySelectorAll('.interactivePlanSvg .generatedPlanRpPlank.isGenerated')];if(tiles.length){let timers=[];const cycle=()=>{timers.forEach(clearTimeout);timers=[];tiles.forEach(t=>t.classList.remove('installLit'));const step=Math.max(55,Math.min(110,2600/tiles.length));tiles.forEach((t,i)=>timers.push(setTimeout(()=>t.classList.add('installLit'),i*step)));const end=tiles.length*step+650;timers.push(setTimeout(()=>tiles.forEach(t=>t.classList.remove('installLit')),end));timers.push(setTimeout(cycle,end+850))};cycle()}
if(svg){
const vb=(svg.getAttribute('viewBox')||'0 0 1000 600').trim().split(/\s+/).map(Number);
let base={x:vb[0],y:vb[1],w:vb[2],h:vb[3]},box={...base},drag=null,animFrame=0,inertiaFrame=0,panVX=0,panVY=0;
const rawSetBox=b=>{box={...b};svg.setAttribute('viewBox',[box.x,box.y,box.w,box.h].join(' '))};
const cancelMotion=()=>{if(animFrame){cancelAnimationFrame(animFrame);animFrame=0}if(inertiaFrame){cancelAnimationFrame(inertiaFrame);inertiaFrame=0}panVX=0;panVY=0};
const frictionEase=(t,duration)=>{const friction=.88,frame=16.667,decay=-Math.log(friction)/frame,elapsed=t*duration,n=1-Math.exp(-decay*duration);return n>1e-6?(1-Math.exp(-decay*elapsed))/n:t};
const animateBox=(target,duration=420)=>{cancelMotion();const start={...box},t0=performance.now();const tick=now=>{const t=Math.min(1,(now-t0)/Math.max(1,duration)),e=frictionEase(t,duration);rawSetBox({x:start.x+(target.x-start.x)*e,y:start.y+(target.y-start.y)*e,w:start.w+(target.w-start.w)*e,h:start.h+(target.h-start.h)*e});if(t<1)animFrame=requestAnimationFrame(tick);else{rawSetBox(target);animFrame=0}};animFrame=requestAnimationFrame(tick)};
const boundsTarget=(b,pad=.14)=>{if(!b)return {...base};const w=Math.max(30,b.maxX-b.minX),h=Math.max(30,b.maxY-b.minY),px=w*pad,py=h*pad;return{x:b.minX-px,y:b.minY-py,w:w+px*2,h:h+py*2}};
const geometryBounds=()=>{let out=null;const selectors='.generatedPlanContextWall,.generatedPlanBaseWall,.generatedPlanRpPlank,.generatedPlanFlow,.generatedPlanFlowMotion,.generatedPlanArrowHead';svg.querySelectorAll(selectors).forEach(n=>{try{const b=n.getBBox();if(!b||!isFinite(b.x)||!isFinite(b.y)||b.width<.01&&b.height<.01)return;const x1=b.x,y1=b.y,x2=b.x+b.width,y2=b.y+b.height;if(!out)out={minX:x1,minY:y1,maxX:x2,maxY:y2};else{out.minX=Math.min(out.minX,x1);out.minY=Math.min(out.minY,y1);out.maxX=Math.max(out.maxX,x2);out.maxY=Math.max(out.maxY,y2)}}catch(_){}});return out};
const allGeometryBounds=geometryBounds();if(allGeometryBounds){base=boundsTarget(allGeometryBounds,.16);rawSetBox(base)}
const fitBounds=(b,pad=.14)=>animateBox(boundsTarget(b,pad),420);
const zoomTarget=(factor,cx=.5,cy=.5)=>{const nw=box.w*factor,nh=box.h*factor;return{x:box.x+(box.w-nw)*cx,y:box.y+(box.h-nh)*cy,w:nw,h:nh}};
const zoom=(factor,cx=.5,cy=.5,duration=420)=>animateBox(zoomTarget(factor,cx,cy),duration);
const startInertia=()=>{if(Math.hypot(panVX,panVY)<.002){panVX=panVY=0;return}let last=performance.now();const tick=now=>{const dt=Math.min(34,Math.max(1,now-last));last=now;rawSetBox({x:box.x+panVX*dt,y:box.y+panVY*dt,w:box.w,h:box.h});const f=Math.pow(.88,dt/16.667);panVX*=f;panVY*=f;if(Math.hypot(panVX,panVY)>.001)inertiaFrame=requestAnimationFrame(tick);else{inertiaFrame=0;panVX=panVY=0}};inertiaFrame=requestAnimationFrame(tick)};
const navSurface=planCard||planStage||svg;
const isUiTarget=e=>Boolean(e.target&&e.target.closest&&e.target.closest('.generatedPlanHotspot,.planSegments,.planTools'));
navSurface.addEventListener('wheel',e=>{if(isUiTarget(e))return;e.preventDefault();const r=svg.getBoundingClientRect(),cx=Math.max(0,Math.min(1,(e.clientX-r.left)/Math.max(1,r.width))),cy=Math.max(0,Math.min(1,(e.clientY-r.top)/Math.max(1,r.height)));zoom(e.deltaY>0?1.12:.89,cx,cy,300)},{passive:false});
navSurface.addEventListener('pointerdown',e=>{if(e.button!==0||isUiTarget(e))return;cancelMotion();drag={id:e.pointerId,lastX:e.clientX,lastY:e.clientY,lastT:performance.now()};svg.classList.add('isPanning');try{navSurface.setPointerCapture(e.pointerId)}catch(_){}});
navSurface.addEventListener('pointermove',e=>{if(!drag||e.pointerId!==drag.id)return;const r=svg.getBoundingClientRect(),now=performance.now(),dt=Math.max(1,now-drag.lastT),dx=e.clientX-drag.lastX,dy=e.clientY-drag.lastY,moveX=-dx*box.w/Math.max(1,r.width),moveY=-dy*box.h/Math.max(1,r.height);rawSetBox({x:box.x+moveX,y:box.y+moveY,w:box.w,h:box.h});panVX=moveX/dt;panVY=moveY/dt;drag.lastX=e.clientX;drag.lastY=e.clientY;drag.lastT=now});
const stop=e=>{if(!drag||e&&e.pointerId!==drag.id)return;drag=null;svg.classList.remove('isPanning');startInertia()};navSurface.addEventListener('pointerup',stop);navSurface.addEventListener('pointercancel',stop);
document.getElementById('planZoomIn').onclick=()=>zoom(.8,.5,.5,460);document.getElementById('planZoomOut').onclick=()=>zoom(1.25,.5,.5,460);
const applySegment=id=>{svg.querySelectorAll('[data-install-run-id]').forEach(n=>n.classList.toggle('segmentDimmed',!!id&&n.getAttribute('data-install-run-id')!==id));document.querySelectorAll('.segmentBtn').forEach(b=>b.classList.toggle('isActive',(b.dataset.segment||'')===id));document.querySelectorAll('[data-flow-segment]').forEach(n=>n.classList.toggle('isHidden',!!id&&n.getAttribute('data-flow-segment')!==id));const run=runData.find(r=>r.id===id);if(id&&run)fitBounds(run.visualBounds||run.bounds);else animateBox(base,420)};
document.getElementById('planFit').onclick=()=>applySegment('');document.querySelectorAll('.segmentBtn').forEach(b=>b.onclick=()=>applySegment(b.dataset.segment||''));
}
...
${closeScript}</body></html>`;
    ...
    async function run(button) {
      ...
      if (label) label.textContent = 'GENERATING CUSTOM INSTALLATION GUIDE...';
      else button.textContent = 'GENERATING CUSTOM INSTALLATION GUIDE...';
      if (window.LuxPlanBusy && typeof window.LuxPlanBusy.show === 'function') {
        window.LuxPlanBusy.show('COMPILING CUSTOM INSTALLATION GUIDE...');
      }
      let url = null;
      try {
        const html = await buildInteractiveHtml(projectName);
        if (window.LuxPlanBusy && typeof window.LuxPlanBusy.update === 'function') {
          window.LuxPlanBusy.update('PACKAGING CUSTOM INSTALLATION GUIDE...');
        }
        ...
        alert('The custom installation guide could not be generated.\n\n' + (error?.message || 'Unknown error.'));
      } finally {
        ...
        if (label) label.textContent = originalText || 'GENERATE CUSTOM INSTALLATION GUIDE';
        else button.textContent = originalText || 'GENERATE CUSTOM INSTALLATION GUIDE';
      }
    }
